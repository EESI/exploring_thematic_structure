...
meta <- meta %>% filter(!is.na(AGE_DECADE_RANK), !is.na(SEX)) 
...
meta$SITE <- str_replace(str_replace(meta$BODY_HABITAT,'UBERON:',''),' cavity','')
K <- 50
...
fit2 <- stm(prevalence=~FEMALE + SITE,
            documents=docs,vocab=vocab,K=K,
            data=meta,
            max.em.its=500,init.type='Spectral',
            seed=seed_fit, #645647
...
fit3 <- stm(prevalence=~FEMALE * SITE,
            documents=docs,vocab=vocab,K=K,
            data=meta,
            max.em.its=500,init.type='Spectral',
            seed=seed_fit, #645647
...
fit5 <- stm(prevalence=~FEMALE + SITE,
            content=~FEMALE,
            documents=docs,vocab=vocab,K=K,
            data=meta,
            max.em.its=500,init.type='Spectral',
...
fit6 <- stm(prevalence=~FEMALE * SITE,
            content=~FEMALE, 
            documents=docs,vocab=vocab,K=K,
            data=meta,
            max.em.its=500,init.type='Spectral',
...
dir_name <- paste0('~/Dropbox/stm_microbiome/data_active/AG_site/stm_s97_rarefied_supervised')
dir.create(dir_name,showWarnings=FALSE,recursive=TRUE)
fit_filename <- paste0('fits_K_',K,'.rds')
saveRDS(list(fits=list(fit1,fit2,fit3,fit4,fit5,fit6,fit0),
             docs=docs,
             vocab=vocab,
             meta=meta,
             taxa=taxa,
