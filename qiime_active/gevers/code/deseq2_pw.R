...
PS <- readRDS('~/Dropbox/stm_microbiome/qiime_active/gevers/data_clean.rds')
SAMP <- sample_data(PS)

biom_file <- read_biom('/data/sw1/Dropbox/stm_microbiome/qiime_active/gevers/picked_otus/ko_table.biom')
ko <- as.matrix(biom_data(biom_file))
samp_ids <- intersect(colnames(ko),rownames(SAMP))
y <- ifelse(SAMP[samp_ids,'DIAGNOSIS']=='CD','1','0')
...
mat <- t(mat)
dx <- ifelse(y=='1','cd','ctrl')
dx <- as.data.frame(dx[rownames(mat),,drop=FALSE])
dx$DIAGNOSIS <- factor(dx$DIAGNOSIS,levels=c('cd','ctrl'))

PS <- phyloseq(otu_table(mat,taxa_are_rows=FALSE),sample_data(dx))
DS2 <- phyloseq_to_deseq2(PS, ~ DIAGNOSIS)
...
res <- DESeq2::results(DS2_OUT, cooksCutoff=FALSE, pAdjustMethod='BH', contrast=c('DIAGNOSIS','cd','ctrl'))
