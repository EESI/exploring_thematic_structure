...
OTU <- otu_table(data_mat_ag[,colSums(data_mat_ag) >= min_read_total],taxa_are_rows=TRUE)

metadata_ag <- data.frame(metadata_ag)
rownames(metadata_ag) <- metadata_ag$SampleID
META <- sample_data(metadata_ag[colnames(OTU),])

TAXA <- tax_table(as.matrix(otu_taxa_ag[rownames(OTU),]))
