...
dir_name <- sprintf('~/Dropbox/stm_microbiome/data_active/Gevers_ti/stm_s97_%s_%s_supervised',rare_method,rare_min)
fit_filename <- sprintf('models/model_K_%s_nfits_%s.rds',K,nfits)
beta_filename <- sprintf('beta/beta_K_%s_fit%s.biom',K,m)
kegg_filename <- sprintf('beta/beta_K_%s_fit%s_predicted_metagenome.biom',K,m)
cog_filename <- sprintf('beta/beta_K_%s_fit%s_predicted_cogs.biom',K,m)


dat <- readRDS(file.path(dir_name,fit_filename))

fit <- dat$fits[[paste0('fit',m)]]
meta <- dat$meta

fit$settings$call

out_dir <- file.path(gsub('data_active','output_active',dir_name),'model')
mod_dir <- paste0('fit_',m)
...
for (k in c(25,35,50,75,100,150)){
  heldout_path <- file.path(dir_name,sprintf('heldout/heldout_K_%s_nfits_%s.rds',k,nfits))
  heldout <- readRDS(heldout_path)
  
  print(k)
  print(sapply(heldout$evals, function(x) x$expected.heldout))
  print(names(which.max(sapply(heldout$evals, function(x) x$expected.heldout))))
  cat(rep('\n',3))
}
...
# eff_formula <- as.formula(paste0(c(1,':',K,as.character(fit$settings$call$prevalence)),collapse=''))
# eff <- estimateEffect(eff_formula, fit, meta, uncertainty='Global')
# eff_plot <- plot.estimateEffect(eff, 'PCDAI', model=fit, topics=1:K, method='continuous',ci.level=0)

eff <- estimateEffect(1:K ~ DIAGNOSIS, fit, meta, uncertainty='Global')
eff_plot <- plot.estimateEffect(eff, 'DIAGNOSIS', model=fit, topics=1:K, method='difference',cov.value1=1,cov.value2=0)
topic_sig <- which(sapply(eff_plot$cis,function(x) sum(sign(x))) != 0)

eff_regression <- estimateEffect(1:K ~ PCDAI, fit, meta, uncertainty='Global')
eff_plot_regression <- plot.estimateEffect(eff_regression, 'PCDAI', model=fit, topics=topic_sig, method='continuous')


eff_spline <- estimateEffect(1:K ~ s(PCDAI), fit, meta, uncertainty='Global')
eff_plot_spline <- plot.estimateEffect(eff_spline, 'PCDAI', model=fit, topics=topic_sig, method='continuous')

coef_k <- unlist(eff_plot$means)
coef_k_unnorm <- coef_k
...
data.frame(fit$theta,id=dat$meta$SampleID,dx=dat$meta$DIAGNOSIS,pcdai=dat$meta$PCDAI,row.names=NULL) %>%
  gather(topic,p,-dx,-pcdai,-id) %>%
  ...
  mutate(status=ifelse(pcdai>0,1,0)) %>%
  group_by(status) %>%
...
#          dx == 1, 
#          !is.na(pcdai)) %>%
#   ggplot(aes(x=pcdai,y=p,colour=pcdai)) + 
...
data.frame(fit$theta,id=dat$meta$SampleID,dx=dat$meta$DIAGNOSIS,pcdai=dat$meta$PCDAI,row.names=NULL) %>%
  ...
  labs(colour='Burden',x='Disease Burden (PCDAI)',y='Probability') + 
  scale_colour_distiller(type='seq',palette=7) +
  theme_bw() +
...
meta$PCDAI_BIN <- meta %>% mutate(status=ifelse(PCDAI>0,1,0)) %>%
  group_by(status) %>%
  mutate(bin=ntile(PCDAI,3)) %>%
  ungroup() %>%
  mutate(bin=as.factor(ifelse(DIAGNOSIS == 1,bin,0))) %>%
  dplyr::select(bin) %>%
  unlist()
...
df <- data.frame(x=pca$x[,1],y=pca$x[,2],DIAGNOSIS=as.factor(meta$DIAGNOSIS))
tpca1 <- df %>%
  ggplot(aes(x=x,y=y,colour=DIAGNOSIS)) + 
  geom_point(alpha=1,size=1.5) +
  facet_wrap(~DIAGNOSIS,nrow=1) +
  geom_point(data=dplyr::select(df,-DIAGNOSIS),colour='gray',alpha=.4,size=.8) +
  theme_bw() + labs(x='PC1',y='PC2',colour='DIAGNOSIS') + theme(legend.position='none') +
  ggtitle('PCA: DIAGNOSIS')

df <- data.frame(x=pca$x[,1],y=pca$x[,2],PCDAI_BIN=as.factor(meta$PCDAI_BIN),DIAGNOSIS=meta$DIAGNOSIS) %>% filter(DIAGNOSIS == 1)
tpca2 <- df %>%
  ggplot(aes(x=x,y=y,colour=PCDAI_BIN)) + 
  geom_point(alpha=1,size=1.5) +
  facet_wrap(~PCDAI_BIN,nrow=2) +
  geom_point(data=dplyr::select(df,-PCDAI_BIN),colour='gray',alpha=.4,size=.8) +
  theme_bw() + labs(x='PC1',y='PC2',colour='PCDAI Bin') + theme(legend.position='none') +
  ggtitle('PCA: PCDAI')
...
df <- data.frame(x=tsne$Y[,1],y=tsne$Y[,2],DIAGNOSIS=as.factor(meta$DIAGNOSIS))
tsne1 <- df %>%
  ggplot(aes(x=x,y=y,colour=DIAGNOSIS)) + 
  geom_point(alpha=1,size=1.5) +
  facet_wrap(~DIAGNOSIS,nrow=1) +
  geom_point(data=dplyr::select(df,-DIAGNOSIS),colour='gray',alpha=.4,size=.8) +
  theme_bw() + labs(x='PC1',y='PC2',colour='DIAGNOSIS') + theme(legend.position='none') +
  ggtitle('tsne: DIAGNOSIS')

df <- data.frame(x=tsne$Y[,1],y=tsne$Y[,2],PCDAI_BIN=as.factor(meta$PCDAI_BIN),DIAGNOSIS=meta$DIAGNOSIS) %>% filter(DIAGNOSIS == 1)
tsne2 <- df %>%
  ggplot(aes(x=x,y=y,colour=PCDAI_BIN)) + 
  geom_point(alpha=1,size=1.5) +
  facet_wrap(~PCDAI_BIN,nrow=2) +
...
df <- data.frame(z,dat$counts$meta_clean %>% filter(DIAGNOSIS == 1) %>% dplyr::select(SampleID,PCDAI,PCDAI_RANK)) %>%
