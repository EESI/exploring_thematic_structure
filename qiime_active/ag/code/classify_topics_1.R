#!/usr/bin/env Rscript

args <- commandArgs(trailingOnly=TRUE)
if (length(args) != 1) stop('Please specify number of topics, K.', call.=FALSE)

library(stm)
library(biom)
library(readr)
library(tidyr)
library(dplyr)
library(fastICA)
library(randomForest)
library(stringr)
library(kernlab)
library(Rcpp)
library(parallel)
library(foreach)
library(ape)
library(doSNOW)
library(snow)
library(phyloseq)
library(stm)
library(LDAvis)
library(caret)
library(glmnet)
library(ggplot2)
library(knitr)
library(gridExtra)
library(caret)
library(pROC)


source('~/Dropbox/stm_microbiome/code_active/stm_functions.R')
source('~/Dropbox/stm_microbiome/code_active/nav_froz_fxns_3.R') 
source('~/Dropbox/stm_microbiome/code_active/performance_1.R')
source('~/Dropbox/stm_microbiome/code_active/framework.R')
source('~/Dropbox/stm_microbiome/code_active/stm_ESTEP.R')


K <- as.integer(args[1])

data_dir <- '~/Dropbox/stm_microbiome/qiime_active/ag'
models_dir <- file.path(data_dir,'models',K)
froz_fn <- sprintf('stm_froz_k_%s.rds',K)

DAT <- readRDS(file.path(models_dir,froz_fn))

meta_train <- DAT$meta_train
meta_test <- DAT$meta_test

theta_train <- DAT$theta_train
theta_test <- DAT$theta_test

train_labels <- factor(ifelse(meta_train$diet==1,'V','O'),levels=c('V','O'))
test_labels <- factor(ifelse(meta_test$diet==1,'V','O'),levels=c('V','O'))

ncores <- 10
cl <- makeCluster(ncores,type='SOCK')
registerDoSNOW(cl)

tr_ctrl <- trainControl(method='repeatedCV', number=10, repeats=10, 
                        classProbs=TRUE,
                        sampling='down',
                        summaryFunction=twoClassSummary,
                        allowParallel=TRUE,
                        verboseIter=TRUE)

param_sweep <- expand.grid(mtry=unique(floor(seq(ncol(theta_train)^.25,ncol(theta_train)^.75,length=10))))
out_rf <- train(x=theta_train,
                y=train_labels,
                method='rf',
                ntree=128,
                tuneGrid=param_sweep,
                importance=TRUE,
                proximity=TRUE,
                metric='ROC', 
                maximize=TRUE, 
                trControl=tr_ctrl,
                verbose=TRUE)

stopCluster(cl)

pred_p <- predict(out_rf,theta_test,type='prob')
pred_Y <- factor(colnames(pred_p)[apply(pred_p,1,which.max)],levels=colnames(pred_p))
pred_out <- data.frame(obs=test_labels,
                       pred=pred_Y,
                       pred_p)
two_class_summary <- twoClassSummary(pred_out,lev=levels(pred_out$pred))
confusion_matrix <- caret::confusionMatrix(pred_Y,test_labels)

roc_curve <- roc(response=test_labels, 
                 predictor=pred_p[,1], 
                 levels=rev(unique(test_labels)))

classify_fn <- sprintf('classify_k_%s.rds',K)
saveRDS(list(K=K,
             OTU=DAT$OTU,
             VOCAB=DAT$VOCAB,
             TAX=DAT$TAX,
             theta_train=theta_train,
             theta_test=theta_test,
             train_labels=train_labels,
             test_labels=test_labels,
             tr_ctrl=tr_ctrl,
             param_sweep=param_sweep,
             out_rf=out_rf,
             pred_p=pred_p,
             pred_Y=pred_Y,
             pred_out=pred_out,
             two_class_summary=two_class_summary,
             confusion_matrix=confusion_matrix,
             roc_curve=roc_curve,
             p=DAT$p,
             idx_train=DAT$idx_train,
             meta_train=DAT$meta_train,
             docs_train=DAT$docs_train,
             meta_test=DAT$meta_test,
             docs_test=DAT$docs_test),
        file.path(models_dir,classify_fn))