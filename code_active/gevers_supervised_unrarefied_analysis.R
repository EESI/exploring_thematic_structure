...
data_mat_risk <- t(data_mat_risk)
samp_filt <- rowSums(data_mat_risk) >= rare_min
cat('Filtering', nrow(data_mat_risk)-sum(samp_filt),'from',nrow(data_mat_risk),'samples for falling below',rare_min,'total reads.\n')
data_mat_risk <- data_mat_risk[samp_filt,]


options(scipen=10000)
Y <- rowSums(data_mat_risk > 0)
X <- rowSums(data_mat_risk)
qplot(X,Y,geom='point',alpha=.1) + xlab('Abundance') + ylab('Prevalence') +
   #scale_x_log10() + 
...
pdf('~/MiscOut/unrarsuper01.pdf', height=10, width=20)
...
pdf('~/MiscOut/unrarsuper02.pdf', height=10, width=20)
