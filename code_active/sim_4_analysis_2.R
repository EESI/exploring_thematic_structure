...
df %>% 
  ...
  geom_label_repel(data=. %>% filter(choose == 1,Threshold==sort(unique(Threshold))[length(THRESH)/2]),
...
TIB <- as_tibble(data.frame(OTU,Sample=rownames(OTU))) %>%
  ...
  filter(OTU %in% paste0('otu',OTUS[1,]))
...
TIB %>%
  ggplot() + 
  geom_violin(aes(x=Effect,y=Count)) + geom_jitter(aes(x=Effect,y=Count),size=2)
