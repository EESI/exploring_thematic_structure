...
dump_dir <- '~/Qiime/Children'

uidx <- unique_idx()
dump_dir_files <- paste0(dump_dir,'/stm_comp_oversamp_',uidx,'/kegg',sep='')
...
# set.seed(43)
# idx_train <- createDataPartition(meta$DIAGNOSIS,p=.8,list=TRUE)$Resample1
...
# set.seed(5455)
# idx_train <- createDataPartition(meta$ISOLATION_SOURCE,p=.8,list=TRUE)$Resample1
...
dump_dir_files <- paste0(dump_dir,'/stm_comp_oversamp_',uidx,'/otu',sep='')
dir.create(dump_dir_files, showWarnings = FALSE,recursive=TRUE)
