...
ps <- names(PS)
model <- c('unsup','prev')
model_grid <- expand.grid(ps=ps,model=model,K=K,
...
nc <- ifelse(nrow(model_grid) < 20,nrow(model_grid),20)
cl <- makeCluster(nc,type='SOCK')
clusterExport(cl,c('logisticnormalcpp','gradcpp','lhoodcpp','hpbcpp'))
registerDoSNOW(cl)
OUT <- foreach(i=1:nrow(model_grid),.verbose=TRUE,.errorhandling="pass",.packages=c("Matrix","stm","phyloseq")) %dopar% {
