...
dir_name <- file.path(sprintf('~/Dropbox/stm_microbiome/data_active/Gevers_ti/stm_s97_%s_%s_supervised',rare_method,rare_min),'models')
dir.create(dir_name,showWarnings=FALSE,recursive=TRUE)
fit_filename <- paste0('model_K_',K,'_nfits_',length(ls_fit),'.rds')
dat <- list(fits=fits,
            docs=docs,
            vocab=vocab,
            meta=meta,
...
saveRDS(dat,file.path(dir_name,fit_filename))

beta_name <- file.path(sprintf('~/Dropbox/stm_microbiome/data_active/Gevers_ti/stm_s97_%s_%s_supervised',rare_method,rare_min),'beta')
dir.create(beta_name,showWarnings=FALSE,recursive=TRUE)
for (i in seq_along(dat$fits)) make_beta_biom(dat,i,beta_name)