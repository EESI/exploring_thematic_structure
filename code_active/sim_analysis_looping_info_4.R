...
if (length(args) >= 2) {
  ...
  start <- 1
  end <- nrow(figure_grid)
}

if (length(args) == 3){
  prev_model <- c('unsup','prev')[as.integer(args[3])]
}else{
  prev_model <- 'unsup'
}

dir.create(file.path(sim_dir,'output',prev_model),showWarnings=FALSE,recursive=TRUE)
dir.create(file.path(sim_dir,'figures',prev_model),showWarnings=FALSE,recursive=TRUE)

for (f in seq(start,end,by=1)){
  ...
  model_param_values <- with(LIST_SUBSET[[1]]$model_params,which(model %in% prev_model & K %in% c(MODEL_K)))
  ...
  SIGLIST <- lapply(1:nrow(param_values), function(x) vector(mode='list',length=length(model_param_values)))
  
  pdf(file.path(sim_dir,'figures',prev_model,sprintf('kl_fig_%s.pdf',f)),height=10,width=10)
  ...
  for (p in seq_along(param_values$idx)){
    ...
    for (m in seq_along(model_param_values)){
      ...
      SIGLIST[[p]][[m]] <- M$figures$p1$data
      SIGLIST[[p]][[m]]$sc_m <- param_values[p,]$sc_m
      SIGLIST[[p]][[m]]$sc_p <- param_values[p,]$sc_p
      SIGLIST[[p]][[m]]$model <- M$data$param$ps
      SIGLIST[[p]][[m]]$prevmodel <- M$data$param$model
      ...
      figure_subtitle <- sprintf('SC_M=%s | SC_P=%s | Model=%s | Cov_model=%s',
                                 param_values[p,]$sc_m,param_values[p,]$sc_p,M$data$param$ps,M$data$param$model)
      KLFIG <- data.frame(SCORE,M$figures$p1$data) %>%
        gather(sc,kl,-c(Lower:Sig)) %>%
        mutate(Sig=factor(Sig,levels=c(-2,0,2)),
               sc=factor(sc,levels=paste0('sc',1:(N_SC*3))),
               Topic=as.factor(Topic)) %>%
        group_by(sc) %>%
        mutate(top=dense_rank(kl),
               top_ann=ifelse(top %in% 1:3,'1','0')) %>%
        ggplot() +
        facet_wrap(~sc,ncol=N_SC) +
        geom_hline(yintercept=0) +
        geom_pointrange(aes(x=kl,y=Mean,ymin=Lower,ymax=Upper,color=Sig,alpha=Sig)) +
        geom_label_repel(data=. %>% filter(top_ann == '1'),
                         aes(x=kl,y=Mean,label=Topic,fill=Sig),color='white',fontface='bold') +
        scale_color_manual(values=c('blue','black','red'),drop=FALSE) +
        scale_fill_manual(values=c('blue','black','red'),drop=FALSE) +
        scale_alpha_manual(values=c(1,.5,1),drop=FALSE) +
        theme(legend.position='none',aspect.ratio=1) +
        labs(x='KL Divergence',y='Estimate') +
        ggtitle(figure_title,subtitle=figure_subtitle)
  ...
  FIG <-   P %>%
    ...
    ggtitle(figure_title,subtitle=paste0('Cov_model=',prev_model))
  
  SIGDF <- do.call('rbind',lapply(SIGLIST,function(x) do.call('rbind',x))) %>%
    mutate(model=factor(model,levels=c('PS_EQ','PS_UNEQ','PS_RARE','PS_DESEQ2')))
  
  SIGFIG <-   SIGDF %>%
    ggplot(aes(x=Topic,ymin=Lower,ymax=Upper,color=Sig)) +
    geom_hline(yintercept=0) +
    geom_linerange() +
    facet_grid(sc_p + sc_m ~ model) +
    scale_color_manual(values=c('blue','black','red')) +
    theme(legend.position='none',aspect.ratio=5/K) +
    ggtitle(figure_title,subtitle=paste0('Cov_model=',prev_model))

  FALSEDF <- SIGDF %>%
    group_by(sc_m,sc_p,model) %>%
    summarise(cov_p1=sum(Sig==2),
              cov_n1=sum(Sig==-2),
              cov_0=sum(Sig==0)) %>%
    gather(weight,count,-c(sc_m:model)) %>%
    filter(weight != 'cov_0') 
  
  FALSEFIG <-   FALSEDF %>%
    ggplot(aes(model,count,fill=model)) +
    geom_bar(color='black',stat='identity') + 
    facet_grid(weight ~ sc_p + sc_m,scales='free_y') +
    theme(aspect.ratio=sqrt(K/5),
          legend.position='none',
          axis.text.x=element_text(hjust=1,angle=90)) +
    ggtitle(figure_title,subtitle=paste0('Cov_model=',prev_model))

  
  cat(c(figure_title,paste0('Cov_model=',prev_model),capture.output(FALSEDF %>% arrange(sc_p,sc_m,weight,model) %>% print.data.frame())), 
      file=file.path(sim_dir,'output',prev_model,sprintf('false_tab_%s.txt',f)),
      sep='\n')
  
  pdf(file.path(sim_dir,'figures',prev_model,sprintf('false_fig_%s.pdf',f)),height=7,width=10)
  print(FALSEFIG)
  dev.off()
  
  pdf(file.path(sim_dir,'figures',prev_model,sprintf('sim_fig_%s.pdf',f)),height=10,width=10)
  print(FIG)
  dev.off()
  
  pdf(file.path(sim_dir,'figures',prev_model,sprintf('sig_fig_%s.pdf',f)),height=20,width=floor(10*K/15))
  print(SIGFIG)
  dev.off()
