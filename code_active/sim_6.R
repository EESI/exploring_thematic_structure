...
for (p in 351:NROW(param_grid)){ #NROW(param_grid)
  ...
  n_otu_uneq <- sample(100:min(colSums(OTU_EQ)),n_samp,replace=TRUE)
...
#   hist(log(OTU_EQ[OTU_EQ>0]),500)
  ...
  OTU_EQ <- round(t(t(OTU_EQ)/colSums(OTU_EQ))*min(colSums(OTU_EQ)))
...
#   hist(log(OTU_EQ[OTU_EQ>0]),500)
  ...
  colMeans(OTU_UNEQ==0)
  colSums(OTU_UNEQ)
  mean(colSums(OTU_UNEQ)<rare_min)
  ...
  for (m in seq_along(OUT)){
    ...
    LIST_OUT[[m]] <-   list(figures=list(p1=p1,eff=eff),
                            data=out,
