...
for (param in 1:NROW(params)){
   ...
   prepare_framework_ag_diet(random_seed,K,cn_normalize,content,variable,seq_sim)
   if (save_output) sink()
   ...
   if (!content){
      ...
      out <- eval_labels_ag_diet(save_fit=TRUE,load_fit=FALSE,train_fit$Z_bar,test_fit$Z_bar,train_meta,test_meta,
                            save_dir,save_fit_foldername,save_coef_filename,
                            nc=60,
                            variableb='DIET_TYPE',varb1='O')
      
      output_dump_ag_diet(save_output,variableb='DIET_TYPE')
      ...
      for (b in 1){
         ...
         output_dump_ag_diet(save_output,model,variableb='DIET_TYPE')
