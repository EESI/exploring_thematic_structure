...
params <- expand.grid(K=c(125,200),
                      content=c(FALSE),
                      variable=c('SEX','BODY_HABITAT'))
params <- params %>% 
   filter(!(content == TRUE  & K == 125 & variable == 'BODY_HABITAT'),
          !(content == TRUE  & K == 200),
          !(content == FALSE & variable == 'BODY_HABITAT')) %>%
   arrange(K,variable)
...
for (param in 1:NROW(params)){
   ...
   prepare_framework_ag_site_sex(random_seed,K,cn_normalize,content,variable,seq_sim)
   if (save_output) sink()
   ...
   if (save_output) {
      output_dir <- file.path('~/Dropbox/stm_microbiome/output_active/AG_site_sex',save_fit_foldername)
   ...
   if (!content){
      ...
      out <- eval_labels_ag(save_fit=TRUE,load_fit=FALSE,train_fit$Z_bar,test_fit$Z_bar,train_meta,test_meta,
                            save_dir,save_fit_foldername,save_coef_filename,
                            nc=60,
                            variableb='SEX',variablem='BODY_HABITAT',varb1='male')
      
      output_dump_ag(save_output,variableb='SEX',variablem='BODY_HABITAT')
      string_dump(save_output)
      ...
      for (b in seq_along(fit_frozen)){
         ...
         output_dump_ag(save_output,variableb='SEX',variablem='BODY_HABITAT',model)
         string_dump(save_output,model)
      }
