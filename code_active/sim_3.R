...
if (length(args) != 2) stop("Two seeds must be supplied.", call.=FALSE)
...
seed_general <- as.integer(args[1]) #10
seed_rare <- as.integer(args[2]) #4
...
for (p in 1:NROW(param_grid)){
  ...
  OUT <- foreach(i=1:nrow(model_grid),.verbose=TRUE,.errorhandling="stop",.packages=c("Matrix","stm","phyloseq")) %dopar% {
    ...
    if (mod == 'unsup'){
      fit <- stm(documents=dat$docs,vocab=dat$vocab,K=param$K,
                 data=dat$meta,
  ...
  saveRDS(list(out=LIST_OUT,
               ...
               ps_params=PARAMS,
               seeds=list(general=seed_general,rare=seed_rare)),
