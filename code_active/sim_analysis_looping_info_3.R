...
for (f in seq(start,end,by=1)){
  ...
  pdf(file.path(sim_dir,'figures',sprintf('kl_fig_%s.pdf',f)),height=10,width=10)
  ...
  for (p in seq_along(param_values$idx)){
    ...
    for (m in seq_along(model_param_values)){
      ...
      figure_subtitle <- sprintf('SC_M=%s | SC_P=%s | Model=%s',param_values[p,]$sc_m,param_values[p,]$sc_p,M$data$param$ps)
  ...
  SIGLIST <- do.call('rbind',lapply(SIGLIST,function(x) do.call('rbind',x)))
  SIGFIG <-   SIGLIST %>%
  ...
  pdf(file.path(sim_dir,'figures',sprintf('sig_fig_%s.pdf',f)),height=20,width=floor(10*K/15))
  print(SIGFIG)
  dev.off()
