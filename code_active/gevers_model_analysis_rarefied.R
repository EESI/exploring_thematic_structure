...
dir_name <- '~/Dropbox/stm_microbiome/data_active/Gevers_ti/stm_s97_rarefied_supervised'
...
coef_top <- order(coef_k,decreasing=TRUE)
coef1 <- coef_top[1:5]
coef0 <- rev(coef_top)[1:5]


vio1 <- plot_violins1(fit,dat$meta,coef1,1) + ggtitle('Topics Associated with CD+')
vio0 <- plot_violins1(fit,dat$meta,coef0,1) + ggtitle('Topics Associated with CD-')
...
df <- data.frame(th,PCDAI=dat$meta$PCDAI,PCDAI_RANK=dat$meta$PCDAI_RANK,DIAGNOSIS=dat$meta$DIAGNOSIS) %>%
  gather(Topic,Abundance,-PCDAI_RANK,-PCDAI,-DIAGNOSIS) %>%
  mutate(Topic=factor(Topic,levels=colnames(th))) %>%
  filter(DIAGNOSIS == 'CD', !is.na(PCDAI)) 

vsage1 <- ggplot(df, aes(PCDAI,Abundance)) + facet_grid(~Topic) + 
  geom_point(alpha=.8,size=1.2,colour='royalblue4') + scale_y_log10() +
  stat_smooth(method='lm',se=FALSE,colour='red') + xlab('PCDAI')  +
  theme_bw() + ggtitle('Topics Associated with CD+')
...
vsage0 <- ggplot(df, aes(PCDAI,Abundance)) + facet_grid(~Topic) + 
  geom_point(alpha=.8,size=1.2,colour='royalblue4') + scale_y_log10() +
  stat_smooth(method='lm',se=FALSE,colour='red') + xlab('PCDAI')  +
  theme_bw() + ggtitle('Topics Associated with CD-')
...
meta$PCDAI_BIN <- meta %>% mutate(status=ifelse(PCDAI>0,1,0)) %>%
  ...
  ungroup() %>%
  mutate(bin=as.factor(ifelse(DIAGNOSIS == 'CD',bin,0))) %>%
  dplyr::select(bin) %>%
  unlist()
...
df <- data.frame(x=pca$x[,1],y=pca$x[,2],PCDAI_BIN=as.factor(meta$PCDAI_BIN),DIAGNOSIS=meta$DIAGNOSIS) %>% filter(DIAGNOSIS == 'CD')
tpca2 <- df %>%
...
pdf(file.path(out_dir,K,mod_dir,'tpca.pdf'), height=10, width=10); gridExtra::grid.arrange(nrow=2,tpca1,tpca2); dev.off()
...
df <- data.frame(x=ica$S[,1],y=ica$S[,2],DIAGNOSIS=as.factor(meta$DIAGNOSIS))
tica1 <- df %>%
  ggplot(aes(x=x,y=y,colour=DIAGNOSIS)) + 
  geom_point(alpha=1,size=1.5) +
  facet_wrap(~DIAGNOSIS,nrow=1) +
  geom_point(data=dplyr::select(df,-DIAGNOSIS),colour='gray',alpha=.4,size=.8) +
  theme_bw() + labs(x='PC1',y='PC2',colour='DIAGNOSIS') + theme(legend.position='none') +
  ggtitle('ica: DIAGNOSIS')

df <- data.frame(x=ica$S[,1],y=ica$S[,2],PCDAI_BIN=as.factor(meta$PCDAI_BIN),DIAGNOSIS=meta$DIAGNOSIS) %>% filter(DIAGNOSIS == 'CD')
tica2 <- df %>%
  ggplot(aes(x=x,y=y,colour=PCDAI_BIN)) + 
  geom_point(alpha=1,size=1.5) +
  facet_wrap(~PCDAI_BIN,nrow=2) +
...
pdf(file.path(out_dir,K,mod_dir,'tica.pdf'), height=10, width=10); gridExtra::grid.arrange(nrow=2,tica1,tica2); dev.off()
...
gridExtra::grid.arrange(nrow=2,tica1,tica2)
pdf(file.path(out_dir,K,mod_dir,'toptica.pdf'), height=10, width=10); gridExtra::grid.arrange(nrow=2,tica1,tica2); dev.off()
# plotly::plot_ly(data.frame(x=ica$S[,1],y=ica$S[,2],z=ica$S[,3]),
#         x=x,y=y,z=z,
#         type='scatter3d',color=rownames(qmat),
#         mode='markers')
